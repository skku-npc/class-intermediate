...
나머지 3개의 부분은, grid의 정 가운데에 ㄱ자 블록을 둠으로써, 한 칸이 x칸인 2^k X 2^k grid로 3개 모두 바꿀 수 있다.
이제 재귀적으로 모두 채울 수 있다.
...
ㅁ ㅌ ㅌ ㅁ
ㅁ ㅁ ㅁ ㅁ
여기서 ㅋ의 위치가 채우면 안되는 칸 이다.
그럼 우상단의 2X2는 귀납적 가정에 의해 채울 수 있다.
나머지 3개의 부분(좌상단, 좌하단 우하단)을 채우기 위해, 중간에 ㄴ자로 블럭을 두면 (ㅌ) 귀납적 가정을 만족하게 되어 채울 수 있다.
//////////

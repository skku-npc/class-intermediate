...
/*
largest함수는 범위 [left, right]에 속하는 히스토그램에서 최대 넓이를 반환하는 함수이다.
(left + right)/2 를 mid라 하면 이라한 직사학형은 
1. mid의 왼편에, 즉[left, mid]범위에 존재 할 수 있다.
2. [mid +1, right]에 존재할 수 있다.
3. mid에 걸쳐 있을 수 있다.
1번과 2번은 재귀적으로 처리할 수 있다.
3번 case 같은 경우 greedy하게 처리할 수 있다.
걸쳐있는 경우, 가장 작은 사각형부터 잡아서 확장해 나간다. (즉 mid, mid + 1의 사각형을 선택한 상태)
이때 한 칸씩 확장해 나가는 식으로 진행한다.
왼쪽으로 한칸 확장하기 vs 오른쪽으로 한칸 확장하기
둘중에서 greedy하게 선택해 나간다.
*/
